...
echo '
         <form id="form-profile" method="POST" action="index.php" enctype="multipart/form-data">
           ...
           <label for="cv">About me: </label><br><textarea type="text" id="cv" name="cv">'.$result->cv.'</textarea><br>
...
// Temp image location
    ...
    require_once('db_config.php');
    $query = "UPDATE users SET fname=?, sname=?, age=?, phone=?, password=?, cv=?, image=? WHERE id=?";
    $stmt = $conn->prepare($query);
    $stmt->execute([$_POST['fname'],$_POST['sname'],$_POST['age'],$_POST['phone'],password_hash($_POST['password'], PASSWORD_DEFAULT),$_POST['cv'],$image_file["name"], $_SESSION['id']]);
    }
